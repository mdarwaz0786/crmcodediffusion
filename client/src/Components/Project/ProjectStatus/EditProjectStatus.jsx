...
const EditProjectStatus = () => {
  ...
  const fetchSingleData = async (id) => {
    try {
      const response = await axios.get(`${base_url}/api/v1/projectStatus/single-projectStatus/${id}`, {
        headers: {
  ...
  const handleUpdate = async (e, id) => {
    ...
    try {
      const response = await axios.put(`${base_url}/api/v1/projectStatus/update-projectStatus/${id}`, updateData, {
        headers: {
          Authorization: validToken,
        },
      });
