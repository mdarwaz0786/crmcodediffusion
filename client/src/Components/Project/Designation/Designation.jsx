...
const Designation = () => {
  ...
  const handleDelete = async (id) => {
    ...
    if (isdelete === "yes") {
      try {
        const response = await axios.delete(`${base_url}/api/v1/designation/delete-designation/${id}`, {
          headers: {
            Authorization: validToken,
          },
        });
  ...
  const exportDesignationListAsExcel = () => {
    const element = document.querySelector("#exportDesignationList");
    if (!element) return;
    const workbook = XLSX.utils.table_to_book(element, { sheet: "Designation List" });
    const excelData = XLSX.write(workbook, { bookType: 'xlsx', type: 'binary' });
    const blob = new Blob([s2ab(excelData)], { type: "application/octet-stream" });
    const url = window.URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'designation-list.xlsx';
                                  ...
                                  <div className="export-dropdwon">
                                    ...
                                    <div className="dropdown-menu  dropdown-menu-end">
                                      <ul>
                                        ...
                                        <li>
                                          <Link to="#" onClick={() => setTimeout(() => { exportDesignationListAsExcel() }, 0)}>
