...
const ProjectCategory = () => {
  ...
  const handleDelete = async (id) => {
    ...
    if (isdelete === "yes") {
      try {
        const response = await axios.delete(`${base_url}/api/v1/projectCategory/delete-projectCategory/${id}`, {
          headers: {
            Authorization: validToken,
          },
        });
  ...
  const exportProjectCategoryListAsExcel = () => {
    if (data?.length === 0) {
    ...
    if (exportData?.length === 0) {
      alert("No project category found to export");
      return;
    };
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "project-category");

    XLSX.writeFile(workbook, `project-category.xlsx`);
  };

  const exportProjectCategoryListAsPdf = () => {
    const element = document.querySelector("#exportProjectCategoryList");
    const options = {
      filename: "project-category.pdf",
      margin: [10, 10, 10, 10],
      html2canvas: {
        useCORS: true,
        scale: 4,
                                  ...
                                  <div className="export-dropdwon">
                                    ...
                                    <div className="dropdown-menu  dropdown-menu-end">
                                      <ul>
                                        ...
                                        <li>
                                          <Link to="#" onClick={() => setTimeout(() => { exportProjectCategoryListAsExcel() }, 0)}>
