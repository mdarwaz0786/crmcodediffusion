...
const TechnologyList = () => {
  ...
  const filedPermissions = team?.role?.permissions?.technology?.fields;
  ...
  const fetchAllTechnologyName = async () => {
    try {
      const response = await axios.get(`${base_url}/api/v1/technology/all-technology`, {
        headers: {
          Authorization: validToken,
      ...
      if (response?.data?.success) {
        setNameData(response?.data?.technology);
      };
    } catch (error) {
      console.log(error.message);
    };
  ...
  const handleDelete = async (id) => {
    ...
    if (isdelete === "yes") {
      try {
        const response = await axios.delete(`${base_url}/api/v1/technology/delete-technology/${id}`, {
          headers: {
            Authorization: validToken,
          },
        });
  ...
  const exportTechnologyListAsExcel = () => {
    const element = document.querySelector("#exportTechnologyList");
    if (!element) return;
    const workbook = XLSX.utils.table_to_book(element, { sheet: "Technology List" });
    const excelData = XLSX.write(workbook, { bookType: 'xlsx', type: 'binary' });
    const blob = new Blob([s2ab(excelData)], { type: "application/octet-stream" });
    const url = window.URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'technology-list.xlsx';
  ...
  const exportTechnologyListAsPdf = () => {
    const element = document.querySelector("#exportTechnologyList");
    const options = {
      filename: "technology-list.pdf",
      margin: [10, 10, 10, 10],
      html2canvas: {
        useCORS: true,
        scale: 4,
  ...
  return (
    <>
      ...
      <div className="page-wrapper">
        <div className="content" id="exportTechnologyList">
          <div className="row">
            <div className="col-md-12">
              ...
              <div className="page-header">
                <div className="row align-items-center">
                  <div className="col-4">
                    <h4 className="page-title">Technologies<span className="count-title">{total}</span></h4>
                  </div>
                    ...
                    <div className="row">
                      <div className="col-md-5 col-sm-4">
                        <div className="form-wrap icon-form">
                          ...
                          <input type="text" className="form-control" placeholder="Search Technology" value={filters.search} onChange={(e) => setFilters((prev) => ({ ...prev, search: e.target.value, page: 1 }))} />
                        </div>
                      </div>
                                ...
                                <li>
                                  <div className="export-dropdwon">
                                    ...
                                    <div className="dropdown-menu  dropdown-menu-end">
                                      <ul>
                                        <li>
                                          <Link to="#" onClick={() => setTimeout(() => { exportTechnologyListAsPdf() }, 0)}>
                                            <i className="ti ti-file-type-pdf text-danger" />
                                            Export as PDF
                                          </Link>
                                        </li>
                                        <li>
                                          <Link to="#" onClick={() => setTimeout(() => { exportTechnologyListAsExcel() }, 0)}>
                                ...
                                <div className="accordion" id="accordionExample">
                                  <div className="filter-set-content">
                                    <div className="filter-set-content-head">
                                      <Link to="#" data-bs-toggle="collapse" data-bs-target="#collapseTwo" aria-expanded="false" aria-controls="collapseTwo">Technology Name</Link>
                                    </div>
                                    <div className="filter-set-contents accordion-collapse collapse show" id="collapseTwo" data-bs-parent="#accordionExample">
                                      <div className="filter-content-list">
                                        <div className="form-wrap icon-form">
                                          <span className="form-icon"><i className="ti ti-search" /></span>
                                          <input type="text" className="form-control" placeholder="Search Technology Name" onChange={(e) => setName(e.target.value)} />
                                        </div>
                                        <ul>
                                          {
                                            nameData?.map((n) => (
                              ...
                              <td>
                                <div className="table-action">
                                  ...
                                  <div className="dropdown-menu dropdown-menu-right">
                                    {
                                      (permissions?.update) && (
                                        <Link to={`/edit-technology/${d?._id}`} className="dropdown-item">
