...
const EditDepartment = () => {
  ...
  const fieldPermissions = team?.role?.permissions?.department?.fields;
  const permissions = team?.role?.permissions?.department;

  const fetchSingleData = async (id) => {
    try {
      const response = await axios.get(`${base_url}/api/v1/department/single-department/${id}`, {
        headers: {
  ...
  const handleUpdate = async (e, id) => {
    ...
    try {
      const response = await axios.put(`${base_url}/api/v1/department/update-department/${id}`, updateData, {
        headers: {
          Authorization: validToken,
        },
      });
