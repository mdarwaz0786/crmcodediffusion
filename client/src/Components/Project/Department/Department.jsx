...
const Department = () => {
  ...
  const permissions = team?.role?.permissions?.department;
  const filedPermissions = team?.role?.permissions?.department?.fields;
  ...
  const fetchAllDepartmentName = async () => {
    try {
      const response = await axios.get(`${base_url}/api/v1/department/all-department`, {
        headers: {
          Authorization: validToken,
      ...
      if (response?.data?.success) {
        setNameData(response?.data?.department);
      };
    } catch (error) {
      console.log(error.message);
    };
  ...
  const handleDelete = async (id) => {
    ...
    if (isdelete === "yes") {
      try {
        const response = await axios.delete(`${base_url}/api/v1/department/delete-department/${id}`, {
          headers: {
            Authorization: validToken,
          },
        });
  ...
  const exportDepartmentListAsPdf = () => {
    const element = document.querySelector("#exportDepartmentList");
    const options = {
  ...
  const exportDepartmentListAsExcel = () => {
    if (data?.length === 0) {
    ...
    if (exportData?.length === 0) {
      alert("No department found to export");
      return;
    };
  ...
  return (
    <>
      ...
      <div className="page-wrapper">
        <div className="content" id="exportDepartmentList">
          <div className="row">
            <div className="col-md-12">
              ...
              <div className="page-header">
                <div className="row align-items-center">
                  <div className="col-4">
                    <h4 className="page-title">Departments<span className="count-title">{total}</span></h4>
                  </div>
                    ...
                    <div className="row">
                      <div className="col-md-5 col-sm-4">
                        <div className="form-wrap icon-form">
                          ...
                          <input type="text" className="form-control" placeholder="Search Department" value={filters.search} onChange={(e) => setFilters((prev) => ({ ...prev, search: e.target.value, page: 1 }))} />
                        </div>
                      </div>
                                ...
                                <li>
                                  <div className="export-dropdwon">
                                    ...
                                    <div className="dropdown-menu  dropdown-menu-end">
                                      <ul>
                                        <li>
                                          <Link to="#" onClick={() => setTimeout(() => { exportDepartmentListAsPdf() }, 0)}>
                                            <i className="ti ti-file-type-pdf text-danger" />
                                            Export as PDF
                                          </Link>
                                        </li>
                                        <li>
                                          <Link to="#" onClick={() => setTimeout(() => { exportDepartmentListAsExcel() }, 0)}>
                                ...
                                <div className="accordion" id="accordionExample">
                                  <div className="filter-set-content">
                                    <div className="filter-set-content-head">
                                      <Link to="#" data-bs-toggle="collapse" data-bs-target="#collapseTwo" aria-expanded="false" aria-controls="collapseTwo">Department Name</Link>
                                    </div>
                                    <div className="filter-set-contents accordion-collapse collapse show" id="collapseTwo" data-bs-parent="#accordionExample">
                                      <div className="filter-content-list">
                                        <div className="form-wrap icon-form">
                                          <span className="form-icon"><i className="ti ti-search" /></span>
                                          <input type="text" className="form-control" placeholder="Search Department Name" onChange={(e) => setName(e.target.value)} />
                                        </div>
                                        <ul>
                                          {
                                            nameData?.map((n) => (
                              ...
                              <td>
                                <div className="table-action">
                                  ...
                                  <div className="dropdown-menu dropdown-menu-right">
                                    {
                                      (permissions?.update) && (
                                        <Link to={`/edit-department/${d?._id}`} className="dropdown-item">
