...
const Office = () => {
  ...
  const fetchAllData = async () => {
    try {
      ...
      if (response?.data?.success) {
        setData(response?.data?.officeLocation);
  ...
  const fetchAllOfficeName = async () => {
    try {
      const response = await axios.get(`${base_url}/api/v1/officeLocation/all-officeLocation`, {
        headers: {
          Authorization: validToken,
      ...
      if (response?.data?.success) {
        setNameData(response?.data?.officeLocation);
      };
    } catch (error) {
      console.log(error.message);
    };
  ...
  const exportOfficeAsExcel = () => {
    ...
    const exportData = data?.map((entry, index) => {
      return {
        "#": index + 1 || "1",
        "Company Code": entry?.uniqueCode || "N/A",
        "Company Name": entry?.name || "N/A",
        "Address Line 1": entry?.addressLine1 || "N/A",
        "Address Line 2": entry?.addressLine2 || "N/A",
        "Address Line 3": entry?.addressLine3 || "N/A",
      };
    });
  ...
  const exportOfficeAsPdf = () => {
    const element = document.querySelector("#exportOffice");
    const options = {
      filename: "office",
      margin: [10, 10, 10, 10],
      html2canvas: {
  ...
  return (
    <>
      ...
      <div className="page-wrapper">
        <div className="content" id="exportOffice">
          <div className="row">
            <div className="col-md-12">
              ...
              <div className="page-header">
                <div className="row align-items-center">
                  <div className="col-4">
                    <h4 className="page-title">Offices<span className="count-title">{total}</span></h4>
                  </div>
                    ...
                    <div className="row">
                      <div className="col-md-5 col-sm-4">
                        <div className="form-wrap icon-form">
                          ...
                          <input type="text" className="form-control" placeholder="Search Office" value={filters.search} onChange={(e) => setFilters((prev) => ({ ...prev, search: e.target.value, page: 1 }))} />
                        </div>
                      </div>
                      <div className="col-md-7 col-sm-8">
                        <div className="export-list text-sm-end">
                          <ul>
                            <li>
                              <div className="export-dropdwon">
                                ...
                                <div className="dropdown-menu dropdown-menu-end">
                                  <ul>
                                    <li>
                                      <Link to="#" onClick={() => setTimeout(() => { exportOfficeAsPdf() }, 0)}>
                                        <i className="ti ti-file-type-pdf text-danger" />
                                        Export as PDF
                                      </Link>
                                    </li>
                                    <li>
                                      <Link to="#" onClick={() => setTimeout(() => { exportOfficeAsExcel() }, 0)}>
                            ...
                            <li>
                              <Link to="/add-office" className="btn btn-primary">
                                ...
                                <div className="accordion" id="accordionExample">
                                  <div className="filter-set-content">
                                    <div className="filter-set-content-head">
                                      <Link to="#" data-bs-toggle="collapse" data-bs-target="#collapseTwo" aria-expanded="false" aria-controls="collapseTwo">Company Unique Code</Link>
                                    </div>
                                    <div className="filter-set-contents accordion-collapse collapse show" id="collapseTwo" data-bs-parent="#accordionExample">
                                      <div className="filter-content-list">
                                        <div className="form-wrap icon-form">
                                          <span className="form-icon"><i className="ti ti-search" /></span>
                                          <input type="text" className="form-control" placeholder="Search unique code" onChange={(e) => setNameSearch(e.target.value)} />
                                        </div>
                                        <ul>
                                          {
                                            nameData?.map((n) => (
                                                ...
                                                <div className="filter-checks">
                                                  <label className="checkboxs">
                                                    <input
                                                      ...
                                                      name="nameFilter"
                                                      value={n?.uniqueCode}
                                                      checked={filters.nameFilter.includes(n?.uniqueCode)}
                      ...
                      <tbody>
                        {
                          data?.map((d, index) => (
                            <tr key={d?._id}>
                              ...
                              <td><Link to={`/single-office/${d?._id}`}><i className="fas fa-eye"></i></Link></td>
