...
const EditSalary = () => {
  ...
  const fetchSalaryDetails = async (id) => {
    try {
      ...
      if (response?.data?.success) {
        const { employee, month, year, amountPaid } = response.data.data;
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(`${base_url}/api/v1/salary/update-salary/${id}`,
        { amountPaid, month, year, employee: selectedEmployee },
        {
          headers: {
            Authorization: validToken,
          },
      ...
      console.error("Error while updating salary:", error.message);
      toast.error("Error while updating salary record");
    };
  };
