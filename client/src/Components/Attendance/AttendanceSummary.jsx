...
const AttendanceSummary = ({ attendance }) => {
  ...
  return (
    <div style={summaryStyle} className="attendance-summary">
      ...
      <div style={itemStyle}><span style={boldText}>Total Shortfall Hours:</span> <span>{attendance?.employeeShortfallHours?.charAt(0) === "+" ? "+" + formatTimeToHoursMinutes(attendance?.employeeShortfallHours) : formatTimeToHoursMinutes(attendance?.employeeShortfallHours)}</span></div>
