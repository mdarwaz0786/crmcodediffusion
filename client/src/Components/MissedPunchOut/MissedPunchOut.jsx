...
const MissedPunchOut = () => {
  ...
  const fetchAllData = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${base_url}/api/v1/missedPunchOut/all-missedPunchOut`, {
        headers: {
          Authorization: validToken,
        },
        params: {
  ...
  const exportMissedPunchOutListAsExcel = () => {
    ...
    const exportData = data?.map((entry, index) => ({
      ...
      "Punch Out Time": `${entry?.punchOutTime}` || "N/A",
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "MissedPunchOut");

    XLSX.writeFile(workbook, `${filters.month || ""}-${filters.year || ""}-${singleEmployee?.name || ""}-Missed-Punch-Out.xlsx`);
  };

  const exportMissedPunchOutListAsPdf = () => {
    const element = document.querySelector("#exportMissedPunchOutList");
    const options = {
      filename: `${filters.month || ""}-${filters.year || ""}-${singleEmployee?.name || ""}-Missed-Punch-Out`,
      margin: [10, 10, 10, 10],
      html2canvas: {
        useCORS: true,
        scale: 4,
  ...
  const handleUpdateStatus = async (e, id) => {
    ...
    try {
      ...
      const response = await axios.put(`${base_url}/api/v1/missedPunchOut/update-missedPunchOut/${id}`,
  ...
  return (
    <>
      ...
      <div className="page-wrapper">
        <div className="content" id="exportMissedPunchOutList">
          <div className="row">
            <div className="col-md-12">
              ...
              <div className="page-header">
                <div className="row align-items-center">
                  <div className="col-4">
                    <h4 className="page-title">Missed Punch Out<span className="count-title">{total}</span></h4>
                  </div>
                        ...
                        <li>
                          ...
                          <div className="export-dropdwon">
                            ...
                            <div className="dropdown-menu  dropdown-menu-end">
                              <ul>
                                <li>
                                  <Link to="#" onClick={() => setTimeout(() => { exportMissedPunchOutListAsPdf() }, 0)}>
                                    <i className="ti ti-file-type-pdf text-danger" />
                                    Export as PDF
                                  </Link>
                                </li>
                                <li>
                                  <Link to="#" onClick={() => setTimeout(() => { exportMissedPunchOutListAsExcel() }, 0)}>
