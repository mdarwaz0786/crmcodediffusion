...
const AddOnService = () => {
  ...
  const fetchAllData = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${base_url}/api/v1/addOnService/all-addOnService`, {
        headers: {
  ...
  const fetchAllServiceName = async () => {
    try {
  ...
  const handleDelete = async (id) => {
    ...
    if (isdelete === "yes") {
      try {
        const response = await axios.delete(`${base_url}/api/v1/addOnService/delete-addOnService/${id}`, {
          headers: {
            Authorization: validToken,
          },
        });
        ...
        console.log("Error while deleting addOnService:", error.message);
        toast.error("Error while deleting");
      };
    };
  };

  const exportAddOnServiceAsExcel = () => {
    if (data?.length === 0) {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "addOnService");

    XLSX.writeFile(workbook, `addOnService.xlsx`);
  };

  const exportAddOnServiceAsPdf = () => {
    const element = document.querySelector("#exportAddOnService");
    const options = {
      filename: "addOnService",
      margin: [10, 10, 10, 10],
      html2canvas: {
        useCORS: true,
        scale: 4,
  ...
  return (
    <>
      ...
      <div className="page-wrapper">
        <div className="content" id="exportAddOnService">
          <div className="row">
            <div className="col-md-12">
              ...
              <div className="page-header">
                <div className="row align-items-center">
                  <div className="col-4">
                    <h4 className="page-title">Add On Services<span className="count-title">{total}</span></h4>
                  </div>
                    ...
                    <div className="row">
                      <div className="col-md-5 col-sm-4">
                        <div className="form-wrap icon-form">
                          ...
                          <input type="text" className="form-control" placeholder="Search Add On Service" value={filters.search} onChange={(e) => setFilters((prev) => ({ ...prev, search: e.target.value, page: 1 }))} />
                        </div>
                      </div>
                            ...
                            <li>
                              <div className="export-dropdwon">
                                ...
                                <div className="dropdown-menu  dropdown-menu-end">
                                  <ul>
                                    <li>
                                      <Link to="#" onClick={() => setTimeout(() => { exportAddOnServiceAsPdf() }, 0)}>
                                        <i className="ti ti-file-type-pdf text-danger" />
                                        Export as PDF
                                      </Link>
                                    </li>
                                    <li>
                                      <Link to="#" onClick={() => setTimeout(() => { exportAddOnServiceAsExcel() }, 0)}>
                                ...
                                <div className="accordion" id="accordionExample">
                                  <div className="filter-set-content">
                                    <div className="filter-set-content-head">
                                      <Link to="#" data-bs-toggle="collapse" data-bs-target="#collapseTwo" aria-expanded="false" aria-controls="collapseTwo">Add On Service Name</Link>
                                    </div>
                                    <div className="filter-set-contents accordion-collapse collapse show" id="collapseTwo" data-bs-parent="#accordionExample">
                                      <div className="filter-content-list">
                                        <div className="form-wrap icon-form">
                                          <span className="form-icon"><i className="ti ti-search" /></span>
                                          <input type="text" className="form-control" placeholder="Search Add On Service Name" onChange={(e) => setName(e.target.value)} />
                                        </div>
                                        <ul>
                                          {
                                            nameData?.map((n) => (
                                                ...
                                                <div className="filter-checks">
                                                  <label className="checkboxs">
                                                    <input
                                                      ...
                                                      name="nameFilter"
                                                      value={n?._id}
                                                      checked={filters.nameFilter.includes(n?._id)}
                      ...
                      <tbody>
                        {
                          data?.map((d, index) => (
                            <tr key={d?._id}>
                              ...
                              <td>{d?.serviceName?.name}</td>
                              <td>{d?.projectName?.projectName}</td>
                              <td>{d?.clientName?.name}</td>
                              <td>{d?.totalProjectCost}</td>
                              <td>
                                <div className="table-action">
                                  ...
                                  <div className="dropdown-menu dropdown-menu-right">
                                    <Link to={`/edit-add-on-service/${d?._id}`} className="dropdown-item">
